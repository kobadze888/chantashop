...
// ... (Logo Components იგივე რჩება: LogoBOG, LogoTBC, etc.) ...
const LogoBOG = () => (
...
const slugToNameMap: Record<string, string> = {
    ...
    // ✅ Georgian
    'shavi': 'შავი', 'tetri': 'თეთრი', 'lurji': 'ლურჯი', 'tsiteli': 'წითელი', 'beji': 'ბეჟი',
    'yavisferi': 'ყავისფერი', 'vardisferi': 'ვარდისფერი', 'mwvane': 'მწვანე', 'stafilosferi': 'ნარინჯისფერი',
    'yviteli': 'ყვითელი', 'rcuxi': 'რუხი', 'nacrisferi': 'ნაცრისფერი', 'cisferi': 'ცისფერი',
    'muqi_lurji': 'მუქი ლურჯი', 'vercxlisferi': 'ვერცხლისფერი', 'oqrosferi': 'ოქროსფერი',
    'iasamnisferi': 'იასამნისფერი', 'kanisferi': 'ხორცისფერი',
};
...
const getColorHex = (slug: string) => {
    ...
    const colorMap: Record<string, string> = {
        ...
        'tsiteli': '#DC2626', 'witeli': '#DC2626', 'red': '#DC2626', 'krasnyj': '#DC2626', 'krasnyy': '#DC2626',
        'beji': '#F5F5DC', 'beige': '#F5F5DC', 'bezhevyj': '#F5F5DC', 'bezhevyy': '#F5F5DC', 'bejevi': '#F5F5DC',
...
// დამხმარე ფუნქცია პროცენტის დასათვლელად
const calculateDiscount = (regular: string | null | undefined, sale: string | null | undefined) => {
    if (!regular || !sale) return 0;
    const reg = parseFloat(regular.replace(/[^0-9.]/g, ''));
    const sal = parseFloat(sale.replace(/[^0-9.]/g, ''));
    if (reg <= 0 || sal <= 0) return 0;
...
export default function ProductInfo({ product, locale = 'ka' }: ProductInfoProps) {
    ...
    const discountPercent = calculateDiscount(regularPrice, isSale);
    ...
    const handleBuyNow = () => {
        if (!isBuyNowDisabled) {
            ...
            for (let i = 0; i < quantity; i++) {
                const msgPayload = (i === quantity - 1) ? messages : undefined;
    ...
    const handleWishlist = () => {
        const messages = {
            added: tToast('wishlistAdded', { name: product.name }),
            removed: tToast('wishlistRemoved', { name: product.name })
        };
            ...
            <div className="lg:col-span-6 flex flex-col py-2">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {colorOptions.map((option) => {
                                ...
                                const bg = getColorHex(option.slug);
                ...
                <div className="space-y-6">
                    ...
                    {technicalAttributes.length > 0 && (
                        <div className="bg-white rounded-2xl p-1">
                            <h4 className="font-bold text-brand-dark mb-4 text-xs uppercase tracking-widest flex items-center gap-2 opacity-60">
                                <Layers className="w-4 h-4" /> {t('detailedSpecs')}
                            </h4>
                    ...
                    <div className="bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden">
                        ...
                        >
                            <span className="flex items-center gap-2"><Info className="w-4 h-4 opacity-60" /> {t('productDescription')}</span>
                        ...
                        {isDescOpen && (
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: product.shortDescription || t('noDescription') }}
                            />
                        )}
                    </div>
