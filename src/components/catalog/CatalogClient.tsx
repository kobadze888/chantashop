...
export default function CatalogClient({ initialProducts, categories, colors, sizes, locale }: CatalogClientProps) {
  ...
  // --- დინამიური დათვლა (კლიენტის მხარეს) ---
  const getProductCounts = (products: Product[], type: 'category' | 'color' | 'size') => {
    const counts: Record<string, number> = {};
    products.forEach(p => {
        let terms: { slug: string }[] = [];
        if (type === 'category') terms = p.productCategories?.nodes || [];
        else if (type === 'color') {
            const attr = p.attributes?.nodes.find(a => a.name === 'pa_color');
            terms = attr?.options?.map(opt => ({ slug: opt.toLowerCase() })) || [];
        } else if (type === 'size') {
            const attr = p.attributes?.nodes.find(a => a.name === 'pa_masala');
            terms = attr?.options?.map(opt => ({ slug: opt.toLowerCase() })) || [];
        }
        terms.forEach(term => {
  ...
  // 2. ✅ კატეგორიები: ვითვლით **ჩატვირთული** პროდუქტებიდან
  // თუ count > 0, მხოლოდ მაშინ ვაჩვენებთ.
  const allCategoryCounts = useMemo(() => getProductCounts(initialProducts, 'category'), [initialProducts]);
  const availableCategories = useMemo(() => {
    return categories
      .map(c => ({ 
          ...c, 
          count: allCategoryCounts[c.slug.toLowerCase()] || 0 
      }))
      // ✅ აქ ვფილტრავთ: თუ 0-ია, არ გამოჩნდეს (გარდა იმ შემთხვევისა თუ უკვე მონიშნულია)
      .filter(c => c.count > 0 || c.slug === selectedCategory);
  }, [categories, allCategoryCounts, selectedCategory]);

  
  // 3. ✅ ფერები: ვითვლით პროდუქტებიდან, რომლებიც აკმაყოფილებენ კატეგორიას და ფასს
  const productsForAttrs = useMemo(() => 
    initialProducts.filter(p => checkCategory(p, selectedCategory) && checkPrice(p, maxPrice)), 
  [initialProducts, selectedCategory, maxPrice]);

  const colorCounts = useMemo(() => getProductCounts(productsForAttrs, 'color'), [productsForAttrs]);
  const availableColors = useMemo(() => {
    return colors
      .map(c => ({ ...c, count: colorCounts[c.slug.toLowerCase()] || 0 }))
      .filter(c => c.count > 0 || c.slug === selectedColor); // ✅ ვმალავთ ცარიელებს
  }, [colors, colorCounts, selectedColor]);

  
  // 4. ✅ მასალები: იგივე ლოგიკა რაც ფერებზე
  const sizeCounts = useMemo(() => getProductCounts(productsForAttrs, 'size'), [productsForAttrs]);
  const availableSizes = useMemo(() => {
    return sizes
      .map(s => ({ ...s, count: sizeCounts[s.slug.toLowerCase()] || 0 }))
      .filter(s => s.count > 0 || s.slug === selectedSize); // ✅ ვმალავთ ცარიელებს
  }, [sizes, sizeCounts, selectedSize]);
        ...
        <div className={`absolute right-0 top-0 bottom-0 w-[80%] bg-white p-6 overflow-y-auto transform transition-transform duration-300 ${mobileFiltersOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-8">
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                        <label className="flex items-center gap-3 cursor-pointer group col-span-2">
                            <input type="checkbox" checked={selectedCategory === 'all'} onChange={() => setSelectedCategory('all')} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT" />
                            <span className="text-gray-600 group-hover:text-brand-dark transition">ყველა</span>
                        </label>
                        {availableCategories.map((cat) => (
                            <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" checked={selectedCategory === cat.slug} onChange={() => setSelectedCategory(selectedCategory === cat.slug ? 'all' : cat.slug)} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT" />
                ...
                <div>
                    ...
                    <div className="px-2">
                        <input type="range" className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-brand-DEFAULT" min="0" max="5000" step="10" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} />
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {availableSizes.map((size) => (
                                <label key={size.id} className="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" checked={selectedSize === size.slug} onChange={() => setSelectedSize(selectedSize === size.slug ? 'all' : size.slug)} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT" />
      ...
      <div className="container mx-auto px-4 flex gap-12 relative">
        <aside className="hidden md:block w-1/4 space-y-10 sticky top-32 max-h-[calc(100vh-10rem)] overflow-y-auto pr-4 hide-scrollbar">
            <div>
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                    <label className="flex items-center gap-3 cursor-pointer group col-span-2">
                        <input type="checkbox" checked={selectedCategory === 'all'} onChange={() => setSelectedCategory('all')} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT shadow-sm flex-shrink-0" />
                        <span className="text-gray-600 group-hover:text-brand-dark transition font-medium">{locale === 'ka' ? 'ყველა' : 'All'}</span>
                    </label>
                    {availableCategories.map((cat) => (
                        <label key={cat.id} className="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" checked={selectedCategory === cat.slug} onChange={() => setSelectedCategory(selectedCategory === cat.slug ? 'all' : cat.slug)} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT shadow-sm flex-shrink-0" />
            ...
            {availableSizes.length > 0 && (
                <div className="pb-10">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {availableSizes.map((size) => (
                            <label key={size.id} className="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" checked={selectedSize === size.slug} onChange={() => setSelectedSize(selectedSize === size.slug ? 'all' : size.slug)} className="w-5 h-5 rounded border-gray-300 text-brand-DEFAULT focus:ring-brand-DEFAULT shadow-sm flex-shrink-0" />
                                <div className="flex items-center justify-between w-full overflow-hidden">
