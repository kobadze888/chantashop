...
import { Search, ShoppingBag, Menu, X, ChevronRight, Percent, ChevronDown, Heart } from 'lucide-react';
...
export default function Header() {
  ...
  const handleLanguageChange = (nextLocale: string) => {
    startTransition(() => {
      // ✅ ზუსტად იგივე ლოგიკა, რაც კალათაზე მუშაობს
      // pathname-ს გადავცემთ ობიექტში params-ებთან ერთად
      router.replace(
        // @ts-expect-error
        { pathname, params },
        { locale: nextLocale }
  ...
  return (
    <>
      ...
      <header className="fixed w-full top-0 md:top-8 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100/50 transition-all duration-300 shadow-sm" id="navbar">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
            
            <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 text-brand-dark hover:bg-gray-100 rounded-full transition cursor-pointer">
            ...
            <nav className="hidden md:flex gap-10 text-sm font-bold tracking-widest text-brand-dark uppercase">
                <Link href="/" className={`hover:text-brand-DEFAULT transition-colors py-2 relative group ${pathname === '/' ? 'text-brand-DEFAULT' : ''}`}>
                    {t('home')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-DEFAULT transition-all ${pathname === '/' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/shop" className={`hover:text-brand-DEFAULT transition-colors py-2 relative group ${pathname === '/shop' ? 'text-brand-DEFAULT' : ''}`}>
                    {t('shop')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-DEFAULT transition-all ${pathname === '/shop' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/about" className={`hover:text-brand-DEFAULT transition-colors py-2 relative group ${pathname === '/about' ? 'text-brand-DEFAULT' : ''}`}>
                    {t('about')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-DEFAULT transition-all ${pathname === '/about' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/contact" className={`hover:text-brand-DEFAULT transition-colors py-2 relative group ${pathname === '/contact' ? 'text-brand-DEFAULT' : ''}`}>
                    {t('contact')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-DEFAULT transition-all ${pathname === '/contact' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/track-order" className={`hover:text-brand-DEFAULT transition-colors py-2 relative group ${pathname === '/track-order' ? 'text-brand-DEFAULT' : ''}`}>
                    {t('trackOrder')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-DEFAULT transition-all ${pathname === '/track-order' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
            </nav>
            ...
            <div className="flex items-center gap-3">
                <div className="relative hidden md:block" ref={dropdownRef}>
                    <button 
                        ...
                        className="flex items-center justify-between w-24 h-10 px-2 text-xs font-bold text-brand-dark bg-gray-50 border border-gray-200 hover:border-brand-DEFAULT hover:bg-white rounded-lg transition-all duration-200 shadow-sm cursor-pointer"
                    >
                        ...
                        <div className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden w-24 py-1 flex flex-col z-50 animate-fade-in">
                            {availableLanguages.map((lang) => (
                                <button
                                    ...
                                    className="px-3 py-2.5 text-left text-xs font-bold hover:bg-brand-light flex items-center gap-2 transition-colors text-gray-600 hover:text-brand-DEFAULT cursor-pointer"
                                >
                                    <div className="relative w-6 h-4.5 rounded-sm overflow-hidden border border-gray-200 shadow-sm shrink-0">
                ...
                <button className="hidden md:flex p-3 hover:bg-gray-100 rounded-full transition group cursor-pointer">
                    <Search className="w-5 h-5 text-gray-600 group-hover:text-brand-dark" />
                </button>

                <Link href="/wishlist" className="relative p-3 hover:bg-gray-100 rounded-full transition group hidden md:flex cursor-pointer">
                    <Heart className={`w-5 h-5 transition-colors ${wishlistCount > 0 ? 'text-red-500 fill-current' : 'text-gray-600 group-hover:text-red-500'}`} />
                    {wishlistCount > 0 && (
                        <span className="absolute top-1 right-1 bg-red-500 border-2 border-white text-white text-[9px] min-w-[16px] h-[16px] flex items-center justify-center rounded-full font-bold">
                            {wishlistCount}
                        </span>
                    )}
                </Link>
                
                <Link href="/cart" className="relative p-3 bg-brand-dark text-white rounded-full transition hover:bg-brand-DEFAULT group shadow-lg hover:shadow-brand-DEFAULT/30 cursor-pointer">
      ...
      <div className={`fixed inset-0 bg-white z-[100] transform transition-transform duration-300 flex flex-col h-screen ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-white shadow-sm shrink-0">
            ...
            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-100 rounded-full hover:bg-brand-light transition cursor-pointer">
        ...
        <div className="flex flex-col p-6 gap-2 overflow-y-auto h-full bg-white pb-32">
            <nav className="flex flex-col gap-4 text-xl font-bold text-brand-dark">
                <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('home')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
                <Link href="/shop" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('shop')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
                <Link href="/wishlist" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('wishlist')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
                <Link href="/about" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('about')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
                <Link href="/contact" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('contact')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
                <Link href="/track-order" onClick={() => setIsMobileMenuOpen(false)} className="p-5 bg-gray-50 rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center shadow-sm cursor-pointer">
                    {t('trackOrder')} <ChevronRight className="w-5 h-5 opacity-50" />
                </Link>
            </nav>
