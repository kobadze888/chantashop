...
// --- SVG FLAGS (High Quality, 21x16 Proportions) ---

const FlagGE = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 16" className={className} aria-label="GE">
    <defs>
        <clipPath id="clip0_ge">
            <rect width="21" height="16" rx="1" fill="white"></rect>
        </clipPath>
    </defs>
    <g clipPath="url(#clip0_ge)">
        <rect width="21" height="16" fill="white"></rect>
        <rect x="8.92578" width="3.15" height="16" fill="#FF0000"></rect>
        <rect y="6.4" width="21" height="3.2" fill="#FF0000"></rect>
...
const FlagRU = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 16" className={className} preserveAspectRatio="none">
     <g clipPath="url(#clip0_ru)">
        <path fill="#fff" d="M0 0h21v16H0z"/>
        <path fill="#0039a6" d="M0 5.333h21v10.667H0z"/>
        <path fill="#d52b1e" d="M0 10.667h21v5.333H0z"/>
     </g>
      <defs>
  ...
  return (
    <>
      ...
      <div className="hidden md:flex fixed top-0 w-full bg-brand-dark text-white text-[10px] md:text-xs font-bold text-center py-2.5 z-[60] tracking-widest uppercase justify-center items-center gap-2">
          <span>{locale === 'ka' ? 'უფასო მიწოდება 200₾+ შეკვეთებზე' : locale === 'ru' ? 'Бесплатная доставка при заказе от 200₾' : 'Free shipping on orders over 200₾'}</span>
      </div>
        ...
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <nav className="hidden md:flex gap-10 text-sm font-bold tracking-widest text-brand-dark uppercase">
                <Link href="/collection" className="hover:text-brand-DEFAULT transition-colors py-2 relative group">
                    {locale === 'ka' ? 'კოლექცია' : locale === 'ru' ? 'Коллекция' : 'Collection'}
                ...
                <Link href="/brands" className="hover:text-brand-DEFAULT transition-colors py-2 relative group">
                    {locale === 'ka' ? 'ბრენდები' : locale === 'ru' ? 'Бренды' : 'Brands'}
                    <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-brand-DEFAULT transition-all group-hover:w-full"></span>
                </Link>
                ...
                <div className="relative hidden md:block" ref={dropdownRef}>
                    ...
                    >
                        <div className="flex items-center gap-2">
                            {/* Rectangular Flag (4:3 ratio) with small radius */}
        ...
        <div className="flex flex-col p-6 gap-2 overflow-y-auto h-full bg-white pb-32">
            <nav className="flex flex-col gap-4 text-xl font-bold text-brand-dark">
                ...
                <Link href="/sale" onClick={() => setIsMobileMenuOpen(false)} className="p-5 border-2 border-brand-DEFAULT text-brand-DEFAULT rounded-2xl active:bg-brand-DEFAULT active:text-white transition flex justify-between items-center mt-4 shadow-sm">
                    SALE <Percent className="w-5 h-5" />
                </Link>
            </nav>
            ...
            <div className="mt-8 border-t border-gray-100 pt-8">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">
                    {locale === 'ka' ? 'აირჩიეთ ენა' : locale === 'ru' ? 'Выберите язык' : 'Select Language'}
                </h4>
                    ...
                    {languages.map((lang) => (
                        <button
                            ...
                            className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-300 ${
                                locale === lang.code 
                                    ? 'border-brand-DEFAULT bg-brand-light text-brand-dark shadow-md transform scale-105' 
