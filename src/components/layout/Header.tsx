...
import { Search, ShoppingBag, Menu, X, ChevronRight, ChevronDown, Heart, Home, LayoutGrid, Info, Phone, Truck } from 'lucide-react';
import { useState, useEffect, useTransition, useRef } from 'react';
import { useCartStore } from '@/store/cartStore';
import { useWishlistStore } from '@/store/wishlistStore'; 
import { useLocale, useTranslations } from 'next-intl';
import Logo from './Logo'; // იმპორტირება გავუკეთეთ ახალ კომპონენტს
import SearchModal from '../search/SearchModal';

// --- დროშების კომპონენტები ---
...
export default function Header() {
  ...
  return (
    <>
      ...
      <header className="fixed w-full top-0 md:top-8 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all duration-300 shadow-sm" id="navbar">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
            
            <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 text-black hover:bg-gray-100 rounded-full transition cursor-pointer">
            ...
            <nav className="hidden md:flex gap-10 text-[15px] font-medium text-black">
                ...
                <Link href="/shop" className={`hover:text-[#db2777] transition-colors py-2 relative group ${pathname === '/shop' ? 'text-[#db2777]' : ''}`}>
                    {t('shop')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-[#db2777] transition-all ${pathname === '/shop' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/about" className={`hover:text-[#db2777] transition-colors py-2 relative group ${pathname === '/about' ? 'text-[#db2777]' : ''}`}>
                    {t('about')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-[#db2777] transition-all ${pathname === '/about' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/contact" className={`hover:text-[#db2777] transition-colors py-2 relative group ${pathname === '/contact' ? 'text-[#db2777]' : ''}`}>
                    {t('contact')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-[#db2777] transition-all ${pathname === '/contact' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
                <Link href="/track-order" className={`hover:text-[#db2777] transition-colors py-2 relative group ${pathname === '/track-order' ? 'text-[#db2777]' : ''}`}>
                    {t('trackOrder')}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-[#db2777] transition-all ${pathname === '/track-order' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </Link>
            </nav>
            ...
            <div className="flex items-center gap-3">
                <div className="relative hidden md:block" ref={dropdownRef}>
                    <button 
                        ...
                        className="flex items-center justify-between w-24 h-10 px-2 text-xs font-bold text-black bg-gray-50 border border-gray-200 hover:border-[#db2777] hover:bg-white rounded-lg transition-all duration-200 shadow-sm cursor-pointer"
                    >
                        ...
                        <div className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden w-24 py-1 flex flex-col z-50 animate-fade-in">
                            {availableLanguages.map((lang) => (
                                <button
                                    ...
                                    className="px-3 py-2.5 text-left text-xs font-bold hover:bg-pink-50 flex items-center gap-2 transition-colors text-gray-600 hover:text-[#db2777] cursor-pointer"
                                >
                                    <div className="relative w-6 h-4.5 rounded-sm overflow-hidden border border-gray-200 shadow-sm shrink-0">
                ...
                <button
                    ...
                    className="p-3 hover:bg-gray-100 rounded-full transition group cursor-pointer flex"
                >
                ...
                <Link href="/wishlist" className="relative p-3 hover:bg-gray-100 rounded-full transition group hidden md:flex cursor-pointer">
                    <Heart className={`w-5 h-5 transition-colors ${wishlistCount > 0 ? 'text-[#db2777] fill-current' : 'text-gray-600 group-hover:text-[#db2777]'}`} />
                    {wishlistCount > 0 && (
                        <span className="absolute top-1 right-1 bg-[#db2777] border-2 border-white text-white text-[9px] min-w-[16px] h-[16px] flex items-center justify-center rounded-full font-bold">
                            {wishlistCount}
                        </span>
                    )}
                </Link>
                
                <Link href="/cart" className="relative p-3 bg-black text-white rounded-full transition hover:bg-[#db2777] group shadow-lg hover:shadow-pink-500/30 cursor-pointer">
                    <div className="relative">
                        <ShoppingBag className="w-5 h-5" />
                        {cartCount > 0 && (
                            <span className="absolute -top-2 -right-2 bg-[#db2777] border-2 border-white text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full font-bold">
                                {cartCount}
                            </span>
                        )}
                    </div>
      ...
      <div className={`fixed inset-0 bg-white z-[100] transform transition-transform duration-300 flex flex-col h-screen ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-white shadow-sm shrink-0">
             <Logo onClick={() => setIsMobileMenuOpen(false)} />

            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-50 rounded-full hover:bg-gray-100 transition cursor-pointer">
                <X className="w-5 h-5 text-black" />
            </button>
        </div>
        
        <div className="flex flex-col px-6 py-6 gap-6 overflow-y-auto h-full bg-white pb-32">
            
            <div>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{tHeader('selectLanguage')}</h4>
                    ...
                    {languages.map((lang) => (
                        <button
                            ...
                            className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all duration-200 cursor-pointer ${
                                locale === lang.code ? 'border-[#db2777] bg-pink-50 text-[#db2777] shadow-sm' : 'border-gray-100 bg-gray-50 text-gray-500 hover:border-gray-300'
                            }`}
                        >
                            <div className="relative w-6 h-4 rounded-sm overflow-hidden border border-gray-200 shadow-sm mb-1.5 shrink-0">
                                <lang.Flag className="absolute inset-0 w-full h-full object-cover" />
                            </div>
            ...
            <nav className="flex flex-col gap-2 text-black">
                <button
                    ...
                    className="p-4 bg-white border border-gray-100 rounded-xl active:bg-gray-50 transition flex items-center justify-between shadow-sm cursor-pointer group w-full text-left"
                >
                    <div className="flex items-center gap-3">
                        <Search className="w-5 h-5 text-gray-400 group-hover:text-[#db2777] transition" />
                        <span className="font-medium text-sm">{tCommon('search')}</span>
                    </div>
